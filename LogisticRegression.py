...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        :param input: symbolic variable that describes the input of thearchitecture (one minibatch)
    :type n_in: int
...
# initialize the biases b as a vector of n_out 0s
    ...
    # class-k
# x is a matrix where row-j represents input training sample-j # b is a vector where element-k represent the free parameter of # hyperplane-k
        self.p_y_given_x = theano.tensor.nnet.softmax_op(theano.dot(input, self.W) + self.b)
...
# end-snippet-1
    ...
    def negative_log_likelihood(self, y):
       ...
       Note: we use the mean instead of the sum so that
        ...
        return -theano.tensor.mean(theano.tensor.log(self.p_y_given_x)[theano.tensor.arange(y.shape[0]), y])  # end-snippet-2
...
# minibatch)
x = theano.tensor.matrix('x') # data, presented as rasterized images
y = theano.tensor.ivector('y') # labels, presented as 1D vector of [int] labels
